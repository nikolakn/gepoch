...
void MainWindow::newEpoch() {
	if (Doc.GetBrojEpoha() > 0) {
		if (QMessageBox::warning(this, tr("Epoch"), tr(
				"Do you want to save document"), tr("Save"), tr("No"))) {
...
void MainWindow::about() {
	QMessageBox::about(this, tr("About Epoch"), tr(
			"The <b>Epoch</b>, simple timeline program.<br>"
				"<br> "
				"Nikola Knezevic, <br>"
...
void MainWindow::createToolBars() {
	...
	connect(godina, SIGNAL(textChanged ()), this, SLOT(godinaChanged()));
	posToolBar->addWidget(godina);
	kategorija = new QComboBox();
	kategorija->addItems(list);
	connect(kategorija, SIGNAL(currentIndexChanged(const QString)), this,
			SLOT(kategorijaChanged(const QString)));
...
void MainWindow::createDockWindows() {
	...
	connect(view, SIGNAL(itemClicked()), this, SLOT(itemClicked()));
	connect(view, SIGNAL(itemDoubleClicked()), this, SLOT(itemDoubleClicked()));
	itemClicked();
}
//move skala
...
void MainWindow::itemClicked() {
	...
	if (sel) {
		...
		if (ep2) {
			NKJD st(sel->GetApStart());
			QString vv = QString("%1:%2").arg(abs(st.GetHour())).arg(abs(
					st.GetMin()));
			QString dd = QString("%1.%2.%3").arg(abs(st.GetGregDay())).arg(abs(
					st.GetGregMonth())).arg(st.GetGregYear());

			property = variantManager->addProperty(QVariant::String, tr(
					"Start date"));
			...
			NKJD st1(sel->GetEndDate());
			st.AddDay(st1.GetJD());
			QString vv1 = QString("%1:%2").arg(abs(st.GetHour())).arg(abs(
					st.GetMin()));
			QString dd1 = QString("%1.%2.%3").arg(abs(st.GetGregDay())).arg(
					abs(st.GetGregMonth())).arg(st.GetGregYear());
			property = variantManager->addProperty(QVariant::String, tr(
					"End date"));
			...
			ap2 = dynamic_cast<NKApsPerson*> (sel);
			if (ap2) {
				property = variantManager->addProperty(QVariant::Bool, tr(
						"is male"));
		...
		property = variantManager->addProperty(
				QtVariantPropertyManager::enumTypeId(), tr("category"));
		...
		property = variantManager->addProperty(
				QtVariantPropertyManager::enumTypeId(), tr("Render"));
		...
		property = variantManager->addProperty(
				QtVariantPropertyManager::enumTypeId(), tr("Visibility"));
		...
		property
				= variantManager->addProperty(QVariant::Color, tr("Pen Color"));
